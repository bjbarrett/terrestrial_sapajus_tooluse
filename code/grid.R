...
#Read the shapefile
grid_110m <- st_read("grid_110m.shp")
...
### Extract number of palm trees for each cell
  ...
  palm_trees_all <- st_read("Palm_nuts_UB_2023_reprojected.shp")
  plot(palm_trees_all)
...
### Reading average availability of stones for each plot
  ...
  # Read data
  average_stones_plot <- read.csv("raw_stones_average_plots.csv")
...
### Extract information from grids to the data
  ...
  # Read data
    heights_data <- read.csv("sertao_heights_2022-2023.csv")
    focals_data <- st_read("sertao_focals_2022-2023_shape.shp")
    nutcracking_data <- st_read("sertao_nutcracking_2022-2023_partial_shape.shp")
...
mapview(grid_22m) + mapview(focals_data_cropped) +  mapview(nutcracking_data_cropped, col.regions ="red") + mapview(palm_trees, col.regions = "green")
