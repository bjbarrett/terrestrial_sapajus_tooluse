...
mzag_1 <- ulam(
    ...
    data=data_list_height, iter=2000, 
    chains=4 , cores=4 , 
    log_lik=FALSE , pars_omit = 'z_id'   
)
...
save(mzag_1 , "model_saves/mzag_1.rds")
...
save(mzag_2 , "model_saves/mzag_2.rds")
...
m1a <- ulam(
    ...
    data=data_list_height2, chains=4, cores=4, iter=1000   
)
...
m1d <- ulam(
    ...
    data=data_list_height_zero, chains=4, cores=4, iter=2000 , control=list(adapt_delta=0.99)  
)

write_rds(m1d , file="model_saves/m1d.rds")
...
#         c(etasq_p,rhosq_p,etasq_l,rhosq_l) ~ exponential(1)
#     ) , 
#     data=data_list_height_zero, chains=4, cores=4, iter=2000 , control=list(adapt_delta=0.97)  
# )
# 
# precis(m1e)

#this gives us an understanding of how palm tree density hiehg in follows with and without nutcracking, data is just the first scan
m1f <- ulam(
    alist(
        height_m|height_m==0 ~ custom( bernoulli_lpmf(1|p) ) , #likelihood loop for terrestrial Bernoulli
        height_m|height_m>0 ~ custom( bernoulli_lpmf(0|p) + gamma_lpdf( height_m | mu/scale , 1/scale) ) ,
    ...
    data=data_list_height2, chains=4, cores=4, iter=2000 , control=list(adapt_delta=0.97)  
)
...
plot(precis(m1fsimp))
plot(precis(m1f))
write_rds(m1fsimp , file="model_saves/m1fsimp.rds")


#this gives us an understanding of how palm tree density height in follows with and without nutcracking
# m1f <- ulam(
#     alist(
#         height_m|height_m==0 ~ custom( bernoulli_lpmf(1|p) ) , #likelihood loop for terrestrial Bernoulli
#         height_m|height_m>0 ~ custom( bernoulli_lpmf(0|p) + gamma_lpdf( height_m | mu/scale , 1/scale) ) ,
...
#         c(etasq_p,rhosq_p,etasq_l,rhosq_l) ~ exponential(1)
#     ) ,
#     data=data_list_height2, chains=4, cores=4, iter=2000 , control=list(adapt_delta=0.99)
# )
# 
# plot(precis(m1f))
# precis(m1f , depth=3 , pars="Rho_id")
# precis(m1f , depth=3 , pars="sigma_id")

write_rds(m1f , file="model_saves/m1f.rds")
...
m1g <- ulam(
    alist(
        ...
        logit(p) <- ap + a_id[id,1] + a_gp[grid_id_follow] + (bNp + a_id[id,3])*nutcracking 
        + (bSp + a_id[id,5])*z_s[grid_id_follow] + (bNxSp + a_id[id,7])*z_s[grid_id_follow]*nutcracking,
        
        log(mu) <- al + a_id[id,2] + a_gl[grid_id_follow] + (bNl + a_id[id,4])*nutcracking 
        + (bSl + a_id[id,6])*z_s[grid_id_follow] +  (bNxSl + a_id[id,8])*z_s[grid_id_follow]*nutcracking,
        
        # priors
        al ~ dnorm( 1 , 2 ),
        c(ap,bNp,bNl,bSp,bSl,bNxSp,bNxSl) ~ dnorm( 0 , 1 ),
        scale ~ dexp(1),
        transpars> matrix[id,8]:a_id <-
            compose_noncentered( sigma_id , L_Rho_id , z_id ),
        matrix[8,id]:z_id ~ normal( 0 , 1 ),
    ...
    data=data_list_height2, chains=4, cores=4, iter=2000 , control=list(adapt_delta=0.995)  
)

write_rds(m1g , file="model_saves/m1g.rds")
m1g <- read_rds(file="/mnt/EAS_ind/bbarrett/terrestrial_sapajus_tooluse/model_saves/m1g.rds")
rethinking::precis(m1g)
rethinking::summary(m1g)
...
m1gsimp <- ulam(
    alist(
        ...
        logit(p) <- ap + a_id[id,1] + (bNp + a_id[id,3])*nutcracking 
        + (bSp + a_id[id,5])*z_s[grid_id_follow] 
        + (bNxSp + a_id[id,7])*z_s[grid_id_follow]*nutcracking,
        
        log(mu) <- al + a_id[id,2] + (bNl + a_id[id,4])*nutcracking 
        + (bSl + a_id[id,6])*z_s[grid_id_follow] 
        + (bNxSl + a_id[id,8])*z_s[grid_id_follow]*nutcracking,
...
write_rds(m1gsimp , file="model_saves/m1gsimp.rds")

readRDS(file="model_saves/m1g.rds")
